...
def draw_tree(filename, addBulk, bulkfile):
    ...
    def write_tree(matrix, names):
        ...
        i = cols - 2
        # @Chengze - edges define the text-formatted graph
        # (parent, child), in this order as directed
        edges = []
        ...
        while i >=0:
            ...
            for j in range(i+1, cols):
                if contains(matrix[:, indeces[j]], matrix[:, indeces[i]]):
                    edges.append((mut_nod[mutations_name[j]].name,
                        mutations_name[i]))
                    ...
                    break
            if not attached:
                edges.append((root.name, mutations_name[i]))
                node = Node(mutations_name[i], root)
                mut_nod[mutations_name[i]] = node
            i -=1
        print_tree(root)

        # @Chengze - write edges to file
        outpath = filename[:-len('.CFMatrix')]
        treefile = open(outpath + '.tre', 'w')
        treefile.write(str(len(edges)) + ' #edges\n')
        for e in edges:
            s, t = e[0], e[1]
            r1, r2 = re.compile('[<mut>]'), re.compile('br[/]br[/]')
            s = re.sub(r1, '', s); s = re.sub(r2, ',', s)
            t = re.sub(r1, '', t); t = re.sub(r2, ',', t)
