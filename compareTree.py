...
def main():
    ...
    for sim in sims:
        ...
        columns = ['method', 'replicate', '# cells', 'missing rate', 'kmax',
                'AD FN', 'AD FP', 'DB FN', 'DB FP', 'SNL FN', 'SNL FP']
        df_list = []

        for i in range(1, rep+1):
            print("On rep\t{}".format(i), end='\r')
            # true tree path
            ref_tree_path = '/home/chengzes/Desktop/Classes/CS598MEB/data/'\
                    '{}/TREES/n7_S{}_k1_tree.tsv'.format(sim, i)
            ...
            for n in ns:
                for m in missing_rates:
                    ...
                    for k in kmax:
                        if k == 0:
                            est_tree_path = '../PhISCS/{}/n={}/{}/rep{}/rep{}.tre'.format(
                                    sim, n, m, i, i)
                        else:
                            est_tree_path = '../PhISCS/{}_kmax=1/n={}/{}/rep{}/rep{}.tre'.format(
                                    sim, n, m, i, i)
        ...
        avg_list = []
        avg_columns = ['method', '# cells', 'missing rate', 'kmax',
                'AD FN', 'AD FP', 'DB FN', 'DB FP', 'SNL FN', 'SNL FP']
            ...
            for m in missing_rates:
                ...
                for k in kmax:
                    row = ['PhISCS', n, m, k] \
                            ...
                            & (df['# cells'] == n) \
                            & (df['missing rate'] == m) \
                            & (df['kmax'] == k), data_columns].mean(axis=0).tolist()
                    avg_list.append(row)
                row = ['modified_PhISCS', n, m, ''] \
                        + df.loc[(df['method'] == 'modified_PhISCS') \
                        & (df['# cells'] == n) \
                        & (df['missing rate'] == m) \
                        & (df['kmax'] == 0), data_columns].mean(axis=0).tolist()
                avg_list.append(row)
